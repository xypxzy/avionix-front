					...
					<div key={elementId}>
						{element.item
							...
							.map(item => (
								<div
									key={item.id}
									className='relative flex size-full flex-col gap-1 rounded border p-1 sm:gap-2 sm:rounded-sm sm:p-2 md:gap-3 md:rounded-xl md:p-3 lg:gap-4 lg:p-4'
								>
									{/* eslint-disable-next-line @next/next/no-img-element */}
									<img
										className='size-full max-h-[107px] max-w-[180px] rounded-xs sm:max-h-[170px] sm:max-w-[322px] md:max-h-[257px] md:max-w-[452px] lg:max-h-[327px] lg:max-w-[542px]'
										src={`assets/specialDeals/${item.image}`}
										alt={item.title}
									/>
									<h4 className='max-w-[180px] text-[12px] sm:text-xs md:max-w-[525px] md:text-base'>
										{item.title}
									</h4>
									<div className='flex flex-col gap-0 md:gap-2'>
										<p className='text-[10px] text-red-700 sm:text-caption md:text-xs lg:text-sm'>
											{item.promotion}
										</p>
