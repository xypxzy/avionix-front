    ...
    return(
        <div className={`flex flex-col gap-5`}>
            <div className={`flex flex-col gap-2`}>
                ...
                <div
                    className={`text-sm font-normal text-primary`}>{departure === '23:59' ? 'Весь день' : `от 00:00 до ${departure}`}</div>
                <Slider className={``} onValueChange={([value]) => setDeparture(minutesToHours(value))}
            ...
            <div className={`flex flex-col gap-2`}>
                ...
                <div
                    className={`text-sm font-normal text-primary`}>{arrival === '23:59' ? 'Весь день' : `от 00:00 до ${arrival}`}</div>
                <Slider onValueChange={([value]) => setArrival(minutesToHours(value))} minStepsBetweenThumbs={1}
                        defaultValue={[1439]} max={1439} step={1}/>
                <div className={'flex justify-between'}>
                    <span className={'block text-sm font-normal text-primary'}>00:00</span>
